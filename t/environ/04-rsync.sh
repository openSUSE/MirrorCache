...
test 3 == $($mc/db/sql "select size from file where name = 'file2.dat'")
...
$mc/curl -H "Accept: */*, application/metalink+xml" /download/folder1/file2.dat | grep '<size>3</size>'

mv $ap7/dt/folder1/file2.dat $ap8/dt/folder1/
echo -n 123456789 > $rs/dt/folder1/file2.dat
...
test 9 == $($mc/db/sql "select max(size) from file where name = 'file2.dat'")

# reports correct size in metalink
$mc/curl -H "Accept: */*, application/metalink+xml" /download/folder1/file2.dat | grep '<size>9</size>'

# still redirects to root because size differs
$mc/curl -I /download/folder1/file2.dat | grep fake.com

# update file on ap8
cp $rs/dt/folder1/file2.dat $ap8/dt/folder1/
...
$mc/curl -I /download/folder1/file3.dat | grep fake.com
...
$mc/backstage/shoot
# now expect to hit
$mc/curl -I /download/folder1/file3.dat | grep -E "$(ap8/print_address)|$($ap7/print_address)"
...
$mc/curl -I /download/folder1/folder11/file1.dat | grep -E "$(ap8/print_address)|$($ap7/print_address)"
